...
func GetLong(n Parent, name string) (int64, error) {
	for _, c := range n.Children() {
		val, ok := c.(*IntegerNode)
		if !ok {
			continue
		...
		if val.Name() == name {
			res, err := strconv.ParseInt(val.Value(), 10, 64)
			if err != nil {
				return 0, err
			}
			return res, nil
...
func GetLongWithDefault(n Parent, name string, def int64) int64 {
	for _, c := range n.Children() {
		val, ok := c.(*IntegerNode)
		if !ok {
			continue
...
func GetShort(n Parent, name string) (int16, error) {
	for _, c := range n.Children() {
		val, ok := c.(*IntegerNode)
		if !ok {
			continue
		...
		if val.Name() == name {
			res, err := strconv.ParseInt(val.Value(), 10, 16)
			if err != nil {
				return 0, err
			}
...
func GetShortWithDefault(n Parent, name string, def int16) int16 {
	for _, c := range n.Children() {
		val, ok := c.(*IntegerNode)
		if !ok {
			continue
...
func GetBooleanWithDefault(n Parent, name string, def bool) bool {
	for _, c := range n.Children() {
		val, ok := c.(*IntegerNode)
		if !ok {
			continue
